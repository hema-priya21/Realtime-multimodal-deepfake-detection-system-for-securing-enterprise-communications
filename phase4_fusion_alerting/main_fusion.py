...
def load_visual_model():
    ...
    if VISUAL_MODEL_PATH.exists():
        ...
        print(f"Loaded visual weights from: {VISUAL_MODEL_PATH}")
    else:
...
def main():
    ...
    # Load local cascade XML with fallback
    ...
    while True:
        ...
        # Mirror video stream horizontally to match webcam orientation
        frame = cv2.flip(frame, 1)

        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        
        # Relaxed detection thresholds for improved low-light face recognition
        faces = face_cascade.detectMultiScale(
            gray, scaleFactor=1.1, minNeighbors=3, minSize=(60, 60)
        )

        # On-screen status indicator when no face is active
        if len(faces) == 0:
            cv2.putText(
                frame, "STATUS: Searching for face...", (20, 40),
        ...
        for (x, y, w, h) in faces:
            ...
            # Placeholder audio score (Replace with Phase 3 integration)
            audio_fake_prob = 0.0
